...
function getStudentsByGrade(subject: string, grade: number): string[] {
  return Object.entries(STUDENT_GRADES)
    .filter(([_, grades]) => grades[subject as keyof typeof grades[0]] === grade)
...
async function updateAssignments() {
  ...
  for (const assignment of assignments) {
    ...
    console.log(`✅ ${subject.toUpperCase()} Grade ${grade}: assigned to ${students.length} students`);
  }
  ...
  console.log(`✅ SEL (Social-Emotional): assigned to all ${selStudents.length} students`);

  db.close();
  console.log("\n✅ All assignments now targeted to correct students!");
  console.log("📋 Each student will see only the assignments matching their per-subject grade level.\n");
}
